...
**Наблюдения:**
- Директории формально отражают многослойность (`Controller`, `Service`, `Entity`, `Infrastructure`, `Contract`, `Integration`, `Message`, `Strategy`).
...
- Контроллеры включают оркестрацию, валидацию и policy-ветки (например квотирование в `AddressSuggestController`), что частично нарушает SRP.

...
final class LruCache implements CacheInterface{
  ...
  private function touch(string $key): void{
    ...
    if($i!==false){ array_splice($this->order, $i, 1); }
    $this->order[] = $key;
    if(count($this->order) > $this->max){
      $old = array_shift($this->order);
      if($old!==null){ unset($this->data[$old]); }
    }
  }
  public function get(string $key): mixed{
    ...
    $this->hits++; $this->touch($key); return $e['v'];
  }
  ...
  public function clear(): void{ $this->data=[]; $this->order=[]; $this->hits=$this->misses=0; }

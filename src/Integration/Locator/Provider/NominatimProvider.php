...
use App\Bridge\Legacy\Contract\Location\ProviderLegacyInterface;
use App\Bridge\Legacy\Contract\Location\ReverseProviderLegacyInterface;

final class NominatimProvider implements ProviderLegacyInterface, ReverseProviderLegacyInterface
{
    ...
    public function geocode(string $q): array
    {
        ...
        $ch = curl_init($url);
        curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER => 1, CURLOPT_TIMEOUT => 4, CURLOPT_USERAGENT => 'locator-phase-38']);
        $body = curl_exec($ch);
        ...
        if ($code >= 200 && $code < 300 && $body) {
            ...
            if (is_array($arr) && isset($arr[0])) {
                return ['lat' => (float) $arr[0]['lat'], 'lon' => (float) $arr[0]['lon'], 'formatted' => (string) ($arr[0]['display_name'] ?? $q)];
            }
        }

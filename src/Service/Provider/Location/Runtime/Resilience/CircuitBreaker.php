...
use App\Locating\ServiceInterface\Provider\Location\Runtime\Resilience\CircuitBreakerInterface;
...
final class CircuitBreaker implements CircuitBreakerInterface
{
    ...
    public function onResult(string $key, bool $ok): string
    {
        ...
        if ($ok) {
            $row['ok'] += 1;
            if ($row['state'] === 'half' && $row['ok'] >= $this->resetOk) {
                $row = ['state' => 'closed','fail' => 0,'ok' => 0,'opened' => 0];
            }
        } else {
            $row['fail'] += 1;
            if ($row['state'] !== 'open' && $row['fail'] >= $this->trip) {
                $row['state'] = 'open';
                $row['opened'] = time();
            }
        }

...
use App\Locating\ServiceInterface\Provider\Location\Runtime\Geo\CellIndexInterface;
...
final class CellIndex implements CellIndexInterface
{
    ...
    public function cover(array $bbox, int $level): array
    {
        $lvl = max(0, min(15, $level));
        $n = 1 << $lvl;
        $lat1 = min((float)$bbox[0], (float)$bbox[2]);
        $lon1 = min((float)$bbox[1], (float)$bbox[3]);
        $lat2 = max((float)$bbox[0], (float)$bbox[2]);
        $lon2 = max((float)$bbox[1], (float)$bbox[3]);
        $x1 = (int)floor(($lon1 + 180.0) / 360.0 * $n);
        $x2 = (int)floor(($lon2 + 180.0) / 360.0 * $n);
        $y1 = (int)floor(($lat1 + 90.0) / 180.0 * $n);
        $y2 = (int)floor(($lat2 + 90.0) / 180.0 * $n);
        $out = [];
    ...
    private function parse(string $id): array
    {
        if (!preg_match('/^L(\d+):(\d+):(\d+)$/', $id, $m)) {
            throw new \InvalidArgumentException('Bad cell id');
        }

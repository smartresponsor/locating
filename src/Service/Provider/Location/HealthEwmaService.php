...
use App\Locating\ServiceInterface\Provider\Location\HealthEwmaInterface;

final class HealthEwmaService implements HealthEwmaInterface
{
    private float $alpha;
    private float $mLatency = 200.0; // ms exponential average
    private float $mError = 0.01;    // error rate exponential average

    public function __construct(float $alpha = 0.2)
    {
        $this->alpha = max(0.01, min(1.0, $alpha));
    }

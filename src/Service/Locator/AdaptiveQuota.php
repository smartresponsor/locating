...
use App\Bridge\Legacy\Service\Location\AdaptiveQuotaLegacyInterface;

final class AdaptiveQuota implements AdaptiveQuotaLegacyInterface
{
    ...
    public function compute(string $tenantId, int $reqUsed, float $costUsed, int $reqLimit, float $costLimit, float $errorRate): array
    {
        ...
        $costAdj = 0.0;
        if ($errorRate > 0.05) {
            $reqAdj = (int) max(-floor($reqLimit * 0.2), -1000);
            $costAdj = -min($costLimit * 0.2, 100.0);
        } elseif ($leftReq > $reqLimit * 0.3 && $leftCost > $costLimit * 0.3 && $errorRate < 0.01) {
            $reqAdj = (int) min(floor($reqLimit * 0.1), 500);
            $costAdj = min($costLimit * 0.1, 50.0);
        }

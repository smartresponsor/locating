...
use App\Bridge\Legacy\Provider\Location\ProviderContractVersionLegacyInterface;

final class ProviderContractVersion implements ProviderContractVersionLegacyInterface
{
    ...
    public function select(string $providerId, string $op, ?string $want): string
    {
        ...
        if (empty($list)) {
            throw new \RuntimeException('No version for provider/op');
        }
        if (null === $want || '' === $want) {
            return end($list) ?: $list[count($list) - 1];
        }
        if (!in_array($want, $list, true)) {
            throw new \InvalidArgumentException('Unsupported version: '.$want);
        }

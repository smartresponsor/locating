...
class ProviderRouter
{
    ...
    private MapboxProvider $mapbox;
    private HereProvider $here;
    private NominatimProvider $nominatim;
    ...
    public function __construct(Env $env, RedisCache $cache)
    {
        $this->env = $env;
        $this->cache = $cache;
        $this->mapbox = new MapboxProvider($env);
        $this->here = new HereProvider($env);
        $this->nominatim = new NominatimProvider($env);
    ...
    {
        $cb = 'mapbox' === $name ? $this->cbMapbox : $this->cbHere;
        if ('nominatim' !== $name) {
            if (!$cb->allow()) {
                return [];
            }
            if (!$this->budget->allow($name)) {
                return [];
            }
        }
        try {

...
use App\Bridge\Legacy\Service\Location\RaceRunnerLegacyInterface;
use Fiber;

final class RaceRunner implements RaceRunnerLegacyInterface
{
    public function run(array $candidate, int $softTimeoutMs = 800): array
    {
        ...
        while (true) {
            ...
            if ($now - $start >= $softTimeoutMs) {
                break;
            }
        }
        if (null === $result) {
            throw new \RuntimeException('RaceRunner timeout or all candidates failed');
        }

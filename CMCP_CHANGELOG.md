...
- Canon020 Normalizer continuation: moved `AddressReverseResultNormalizer` from `Service/Provider/Location` to `Normalizer/Provider/Location` and its typed contract from `ServiceInterface/Provider/Location` to the mirrored `NormalizerInterface/Provider/Location` tree. Updated DI, `OrderedAddressReverseProvider`, tests, Composer lint paths, and LC-12 namespace/declaration validation.
- Verification: all changed/new PHP files pass syntax; PHPStan passes with 0 errors; local `composer canon` passes with LC-12 green and service-family warnings at 25; `git diff --check` passes. The current central `composer gate` executable has changed contract since the previous slice: it now requires an explicit component profile, no Locating profile exists in the current Gating distribution, and it emits aggregated rules instead of Canon000-031. Under the current central run, `layer.typed_class_location` no longer reports `AddressReverseResultNormalizer`; its remaining failures are unrelated pre-existing typed-role classes. This profile/tooling drift is recorded explicitly and is not represented as the old Canon002/007/011/019/020 result set.
- Canon020 Normalizer continuation: moved `AddressNormalizer` from `Service/Address/Location` to `Normalizer/Address/Location` and `AddressNormalizerInterface` to the mirrored `NormalizerInterface/Address/Location` tree. Updated `AddressPipeline`, DI wiring, focused tests, and LC-08 so the local address-path guard recognizes the technical-role-first Normalizer roots and explicitly retires the old Service paths.
- Verification: changed/new PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes with LC-08 green and service-family warnings reduced to 25 across 195 services / 183 interfaces; `cs:check` passes. Current central `composer gate` still lacks a Locating component profile, but `layer.typed_class_location` does not report `AddressNormalizer`; its remaining typed-role failures are `LogEvent` and `ArrayTenantConfigRepository`. Broader gate failures remain pre-existing profile/terminology/mutation-firewall debt and are not represented as slice regressions.
- Canon020 Handler completion: moved `AddressBatchMessageHandler` from `MessageHandler/Batch/Location` to `Handler/Batch/Location` and its typed contract from `MessageHandlerInterface/Batch/Location` to mirrored `HandlerInterface/Batch/Location`. Updated Symfony DI, `InMemoryAddressBatchMessageBus`, all active App-level tests/consumers found by Git grep, and LC-10; the old App-level MessageHandler FQCNs are no longer referenced.
- Verification: changed/new PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes with LC-10 green; current central `composer gate` no longer reports `AddressBatchMessageHandler` in `layer.typed_class_location`; `cs:check` passes with 0 fixable files. This exhausts the previously reported three-item Canon020 typed-role list (`AddressReverseResultNormalizer`, `AddressNormalizer`, `AddressBatchMessageHandler`) while broader Canon006/profile/terminology debt remains.
- Canon006 Repository continuation: moved `ArrayTenantConfigRepository` from `Service/Location/Tenant` to `Repository/Location/Tenant` and moved `TenantConfigRepositoryInterface` to the mirrored `RepositoryInterface/Location/Tenant` tree. Updated `AddressQuotaGuard`, Symfony DI, focused tenant/quota tests, and all active contract consumers. `TenantLimitInterface` remains in its existing service-interface role and is imported by the repository contract.
- Verification: changed/new PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduced from 25 to 24 (194 services / 182 service interfaces); current central `composer gate` no longer reports `ArrayTenantConfigRepository` in `layer.typed_class_location` and inventories one repository file; `cs:check` passes with 0 fixable files. Remaining current central typed-role findings are the two `LogEvent` classes, which require separate semantic reconciliation rather than a blind move.
- Canon006 semantic role repair: reconciled two duplicate `LogEvent` implementations. `ReadModel/Observability/Location/LogEvent` was behaviorally identical to the service implementation and had no active consumers, so it was retired. The retained behavior was renamed from `LogEvent` to `LogEventSerializer` and moved from `Service/Observability/Location` to `Serializer/Observability/Location`; its contract became `SerializerInterface/Observability/Location/LogEventSerializerInterface`. The `toJson()` behavior, redaction, trace/span enrichment, and JSON encoding semantics are unchanged.
- Verification: old Service/ServiceInterface LogEvent FQCNs have no active references; new serializer/interface syntax passes; PHPStan passes with 0 errors; local `composer canon` passes (595 PHP files, 24 service-family warnings, 315 potentially non-canonical names); current central `layer.typed_class_location` is now fully PASSED with no remaining typed-role findings; `cs:check` passes with 0 fixable files. Central gate still fails only on separate terminology/profile/mutation-firewall debt.
- Canon006 Policy continuation: moved `RetentionPolicy` from `Service/Privacy/Location` to `Policy/Privacy/Location` and moved `RetentionPolicyInterface` to the mirrored `PolicyInterface/Privacy/Location` tree. Updated `RetentionSweeper`, its interface, `ErasureJob`, and focused retention tests. The separate runtime `RetentionInterface` remains in place and continues to be implemented by the policy.
- Verification: changed/new PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and LC-07 service-family warnings reduce from 24 to 23 (192 services / 180 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Broader central failures remain limited to separate terminology/profile/mutation-firewall debt.
- Canon006 Policy continuation: moved isolated `CacheTtlPolicy` from `Service/Provider/Location/Cache` to `Policy/Provider/Location/Cache` and moved `CacheTtlPolicyInterface` to the mirrored `PolicyInterface/Provider/Location/Cache` tree. No active runtime consumers were present, so no behavioral rewiring was required.
- Verification: new policy/interface syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and LC-07 service-family warnings reduce from 23 to 22 (191 services / 179 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files.
- Canon006 Policy continuation: moved `AdaptiveTtlPolicy` from `Integration/Provider/Location/Cache` to `Policy/Provider/Location/Cache` and updated its sole `AdaptiveMultiCachedLocator` consumer; behavior is unchanged and the remaining non-English inline comment was normalized to English. During this slice, parallel in-scope changes appeared for `CostAwareAddressReverseSourceCostPolicy` and `CostAwareAddressSuggestionSourceCostPolicy` plus their cost-policy interfaces. Those changes were preserved, inspected, and completed under the same bounded Policy role cluster rather than discarded: implementations now live under `Policy/Provider/Location`, interfaces under mirrored `PolicyInterface/Provider/Location`, and config/order/test/LC-12 references are aligned.
- Verification: all new Policy/PolicyInterface PHP files pass syntax; PHPStan passes with 0 errors after the parallel cost-policy slice reached a stable state; local `composer canon` passes including LC-12 and LC-07 warnings reduce from 22 to 20 (189 services / 177 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. The earlier transient PHPStan/LC-12 failures were captured while the parallel cost-policy move was only partially applied and are not the final acceptance state.
- Canon006 provider cost-policy continuation: moved `CostAwareAddressReverseSourceCostPolicy` and `CostAwareAddressSuggestionSourceCostPolicy` from `Service/Provider/Location` to `Policy/Provider/Location`, with `AddressReverseSourceCostPolicyInterface` and `AddressSuggestionSourceCostPolicyInterface` mirrored under `PolicyInterface/Provider/Location`. Updated source-order consumers, Symfony DI, focused tests, Composer lint paths, and LC-12 policy-root recognition. Behavior and provider-cost signal semantics are unchanged.
- Verification: all owned changed/new PHP files pass syntax; PHPStan passes with 0 errors; local `composer canon` passes with LC-12 green and service-family warnings reduced from 22 to 20 (189 services / 177 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 adaptive TTL policy continuation: moved `AdaptiveTtlPolicy` from `Integration/Provider/Location/Cache` to `Policy/Provider/Location/Cache` and updated its only active consumer, `AdaptiveMultiCachedLocator`, to the policy namespace. Runtime TTL decision behavior is unchanged.
- Verification: the moved policy and decorator pass PHP syntax; the full worktree passes PHPStan with 0 errors, local `composer canon`, current central `layer.typed_class_location`, and `cs:check`. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 provider health-policy continuation: moved `HealthAwareAddressReverseSourceHealthPolicy` and `HealthAwareAddressSuggestionSourceHealthPolicy` from `Service/Provider/Location` to `Policy/Provider/Location`, with `AddressReverseSourceHealthPolicyInterface` and `AddressSuggestionSourceHealthPolicyInterface` mirrored under `PolicyInterface/Provider/Location`. Updated source-order consumers, Symfony DI, focused policy tests, Composer lint paths, and LC-12 paths. Health scoring behavior and provider-health signal semantics are unchanged.
- Verification: owned PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes with LC-12 green and service-family warnings reduced from 20 to 18 (187 services / 175 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 provider quota-policy continuation: moved `QuotaAwareAddressReverseSourceQuotaPolicy` and `QuotaAwareAddressSuggestionSourceQuotaPolicy` from `Service/Provider/Location` to `Policy/Provider/Location`, with `AddressReverseSourceQuotaPolicyInterface` and `AddressSuggestionSourceQuotaPolicyInterface` mirrored under `PolicyInterface/Provider/Location`. Updated source-order consumers, Symfony DI, focused policy tests, Composer lint paths, and LC-12 paths. Quota decision behavior and provider-quota signal semantics are unchanged.
- Verification: owned PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes with LC-12 green and service-family warnings reduced from 18 to 16 (185 services / 173 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime resilience continuation: moved `RetryPolicy` from `Service/Provider/Location/Runtime/Resilience` to `Policy/Provider/Location/Runtime/Resilience` and moved `RetryPolicyInterface` to mirrored `PolicyInterface/...`. Updated `RouterOrchestrator`, the deprecated model wrapper, and focused retry tests while preserving retry/backoff behavior and the compatibility wrapper.
- Verification: owned PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 16 to 15 (184 services / 172 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime resilience continuation: moved `SlaPolicy` from `Service/Provider/Location/Runtime/Resilience` to `Policy/Provider/Location/Runtime/Resilience` and moved `SlaPolicyInterface` to mirrored `PolicyInterface/...`. Updated `FailoverPlanner`, `AdaptiveOrdering`, and `RouterOrchestrator`; SLA weighting semantics are unchanged.
- Verification: owned PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 15 to 14 (183 services / 171 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime resilience continuation: moved isolated `CircuitTuningPolicy` from `Service/Provider/Location/Runtime/Resilience` to `Policy/Provider/Location/Runtime/Resilience` and moved `CircuitTuningPolicyInterface` to mirrored `PolicyInterface/...`. No active runtime consumers were present, so no behavioral rewiring was required.
- Verification: moved policy/interface syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 14 to 13 (182 services / 170 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime resilience continuation: moved isolated `HedgePolicy` from `Service/Provider/Location/Runtime/Resilience` to `Policy/Provider/Location/Runtime/Resilience` and moved `HedgePolicyInterface` to mirrored `PolicyInterface/...`. No active runtime consumers were present; hedge-delay behavior is unchanged.
- Verification: moved policy/interface syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 13 to 12 (181 services / 169 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime resilience continuation: moved isolated `RegionTimeoutPolicy` from `Service/Provider/Location/Runtime/Resilience` to `Policy/Provider/Location/Runtime/Resilience` and moved `RegionTimeoutPolicyInterface` to mirrored `PolicyInterface/...`. No active runtime consumers were present; timeout lookup behavior is unchanged.
- Verification: moved policy/interface syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 12 to 11 (180 services / 168 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime experiment continuation: moved `BanditPolicy` from `Service/Provider/Location/Runtime/Experiment` to `Policy/Provider/Location/Runtime/Experiment` and moved `BanditPolicyInterface` to mirrored `PolicyInterface/...`. Updated `ProviderOrderService` and `ProviderOrderInterface`; selection/update semantics are unchanged.
- Verification: owned PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 11 to 10 (179 services / 167 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime geo continuation: moved isolated `GeoFencePolicy` from `Service/Provider/Location/Runtime/Geo` to `Policy/Provider/Location/Runtime/Geo` and moved `GeoFencePolicyInterface` to mirrored `PolicyInterface/...`. No active runtime consumers were present; geofence decision/allow semantics are unchanged.
- Verification: moved policy/interface syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 10 to 9 (178 services / 166 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.
- Canon006 runtime routing continuation: moved `CostAwarePolicy` from `Service/Provider/Location/Runtime/Routing` to `Policy/Provider/Location/Runtime/Routing` and moved `CostAwarePolicyInterface` to mirrored `PolicyInterface/...`. Updated `AdaptiveOrdering`; cost-aware score semantics are unchanged.
- Verification: owned PHP syntax passes; PHPStan passes with 0 errors; local `composer canon` passes and service-family warnings reduce from 9 to 8 (177 services / 165 service interfaces); current central `layer.typed_class_location` remains PASSED; `cs:check` passes with 0 fixable files. Central gate remains non-green only on separate terminology/profile/mutation-firewall debt.

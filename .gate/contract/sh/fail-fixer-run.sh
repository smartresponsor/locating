...
python3 - <<'PY'
import os, sys, json, hashlib, re, shutil, subprocess

proposal_path = os.environ.get("PROPOSAL_PATH")
...
def is_safe(p: dict) -> bool:
    ...
    if op in {"file.append_lines","file.ensure_exists","path.ensure_dir","chmod.add_x","report.print"}:
        return True
    if op == "file.write_text":
        return p.get("guard","") == "missing_only"
    return False
...
def apply_replace_regex(p):
    path = p["path"]
    if not os.path.exists(path):
        print(f"[gate] skip file.replace_regex (missing) path={path}")
        return
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()
    new = re.sub(p["pattern"], p["replacement"], content, flags=re.MULTILINE)
    if new != content:
        with open(path, "w", encoding="utf-8") as f:
            f.write(new)
        print(f"[gate] applied file.replace_regex path={path}")
    else:
        print(f"[gate] noop file.replace_regex path={path}")
...
def apply_delete(p):
    path = p["path"]
    if not os.path.exists(path):
        print(f"[gate] noop file.delete path={path}")
        return
    if os.path.isdir(path):
        shutil.rmtree(path)
    else:
        os.remove(path)
    print(f"[gate] applied file.delete path={path}")
...
def apply_move(p):
    ...
    if not os.path.exists(src):
        print(f"[gate] skip path.move (missing from) from={src}")
        return
    if os.path.exists(dst) and not overwrite:
        print(f"[gate] skip path.move (target exists, overwrite=false) to={dst}")
        return
    ...
    shutil.move(src, dst)
    print(f"[gate] applied path.move from={src} to={dst}")

def apply_rename(p):
    apply_move({"from":p["from"],"to":p["to"],"overwrite":False})
...
handlers = {
    ...
    "file.replace_regex": apply_replace_regex,
    "file.write_text": apply_write_text,
    "file.delete": apply_delete,
    "path.ensure_dir": apply_ensure_dir,
    "path.move": apply_move,
    "path.rename": apply_rename,
...
for ln in lines:
    ...
    fn = handlers.get(op)
    if not fn:
        print(f"[gate] unknown op={op} (skip)")
        continue
